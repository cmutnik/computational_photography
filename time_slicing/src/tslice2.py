...
class TimeSlice:
    def __init__(self, filePath, fileExt, imRotate=None, verbose=False):
        ...
        if verbose:
            print(
                "Found %d %s files in dir %s"
                % (self.numFiles, self.fileExt, self.filePath)
            )
    ...
    def slice(
        ...
        im_temp = im_temp[
            trim[1] : im_temp.shape[0] - trim[3], trim[0] : im_temp.shape[1] - trim[2]
        ]
        ...
        if verbose:
            ...
            print("Output image size is %i x %i px" % (imShape_ij[1], imShape_ij[0]))
        ...
        for i in range(numSlices):
            ...
            if verbose:
                print("Image number is %i of %i" % ((i * interval) + 1, self.numFiles))
            ...
            im_slice = im_this[slice_TL_i : slice_BR_i + 1, slice_TL_j : slice_BR_j + 1]
            del im_this

            if borderWidth > 0:
                im_slice[0 : 0 + borderWidth, :] = borderCol
                im_slice[
                    im_slice.shape[0] - borderWidth : im_slice.shape[0], :
                ] = borderCol
                im_slice[:, 0 : 0 + borderWidth] = borderCol
                im_slice[
                    :, im_slice.shape[0] - borderWidth : im_slice.shape[1]
                ] = borderCol
